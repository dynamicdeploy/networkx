	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR){
			...
			$.each(data, function(index, item) {
				$('#content').prepend('<tr><td><a href="'+item.url+'" target="_blank">'+item.title+'</a>   <br/>tags: '+item.tagstr+' ::   Posted by '+item.username+' at <a href="/viewpost/'+item.contentId+'">'+item.timestampAsStr+'</a><br/></td></tr>');
			});
			$('#title').val("");
			$('#url').val("");
...
function updateContent() {
	...
	$.ajax({
		...
		success: function(data, textStatus, jqXHR){
			$.each(data, function(index, item) {
				$("#url_"+contentId).text(item.title);
				$("#url_"+contentId).attr('href',item.url);
				$("#tags_"+contentId).text(item.tagstr);
				$("#addcontent").text('Add Content');
				$("#contentform")[0].reset();
				$("#contentform").hide();
			});
...
function showContentStream(data){
	...
	$.each(list, function(index, content) {
		if(index<3){
			...
			$('#content').append('<tr id="_'+id+'"><td><a href="'+content.url+'" id="url_'+id+'" target="_blank">'+content.title+'</a> '+ ti +'<br/>tags:<span id="tags_'+id+'">'+content.tagstr+'</span> ::   Posted by '+content.username+' at <a href="/viewpost/'+content.contentId+'">'+content.timestampAsStr+'</a> </td></tr>');
		}
	});
	...
	$(document).on('keyup.autocomplete','input[name="tagstr"]', function(event){
		...
		$(this).autocomplete({
			...
			source: function( request, response ) {
				$.getJSON(  "/tag/" + extractLast( request.term.toLowerCase() ), response );
			},
			search: function() {
				// custom minLength
...
function searchProducts(){
	...
	$(document).on('keyup.autocomplete','input[name="product"]', function(event){
		...
		$(this).autocomplete({
			...
			source: function( request, response ) {
				$.getJSON(  "/productsearch/" + extractLast( request.term.toLowerCase() ), response );
			},
			search: function() {
				// custom minLength
			},
...
function loadMoArProducts(){
	$('ul#productlist').jscroll({
		loadingHtml: '<div align="center" style="margin: 20px 0 20px 0"><li id="feed_load"><img src="/img/loader.gif" alt="ajax loading indicator" class="ajax-loader"> &nbsp; Loading more products...</li></div>',
		padding: 10,
